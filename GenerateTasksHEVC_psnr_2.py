...
def create_yuv_filename(s):
    YUV_FILENAME_TEMPLATE = "../../%s/%s_%dx%d_%d_%dbit.yuv"
    return YUV_FILENAME_TEMPLATE%(YUV_PATH,s2name[s],s2resolution[s][0],s2resolution[s][1],s2framerate[s],s2bitdepth[s])
...
def create_commandline_psnr():
    #Create task id name form sequence number and qp value
    COMMAND_LINE_TEMPLATE = "../../%s/psnr.out"
    return COMMAND_LINE_TEMPLATE%(BIN_PATH)

def create_argline_psnr_org(s,qp):
    #Create task id name form sequence number and qp value
    ARGLINE_TEMPLATE = "-i1 %s -i2 %s -dx %d -dy %d"
    return ARGLINE_TEMPLATE%(create_yuv_filename(s),create_bitstream_filename(s,qp),s2resolution[s][0],s2resolution[s][1])
...
def prepare_paths_or_del(remove_or_create):
    ...
    if (GEN_TRANSCODE|GEN_PSNR|GEN_ALL_CFG|remove_or_create):
